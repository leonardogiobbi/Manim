from tkinter import W, WRITABLE
...
class Pitagora(Scene):
    def construct(self):
        ...
        # testo ABC
        A = always_redraw(lambda: Tex('A').next_to(d1, LEFT+DOWN))
        B = always_redraw(lambda: Tex('B').next_to(d2, DOWN))
        C = always_redraw(lambda: Tex('C').next_to(d3, LEFT))
        ...
        # ds1 = Dot([tri_side1.get_length, d2.get_y(), 0])
        # ds2 = Dot([tri_side1.get_length, d3.get_y(), 0])
        ...
        # D
        D = always_redraw(lambda: Tex('D').next_to(ds1, RIGHT+DOWN))
        ...
        self.play(Write(VGroup(
            ...
            always_redraw(lambda: Tex("A'").next_to(ds2)), 
            always_redraw(lambda: Tex("B'").next_to(d2_copy))
            )))
        self.play(Create(always_redraw(lambda: Line(d2_copy, d2)), run_time=0.7))

        self.wait()

        self.play(VGroup(d1, d2, d3, ds1, ds2, d1_copy, d2_copy, line1, line2, A, C).animate.to_edge(LEFT))
        ...
        quad2 = always_redraw(lambda: Polygon(
            [d2.get_x(), d2.get_y(), 0], 
            [ds1.get_x(), ds1.get_y(), 0],
